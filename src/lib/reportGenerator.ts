...
export async function generateReport(data: ReportData): Promise<Blob> {
  ...
  const overallHealth = severityCounts.critical > 0 ? "Crítico" : 
                       severityCounts.high > 0 ? "Grave" : 
                       severityCounts.medium > 0 ? "Bueno" : "Excelente";
      ...
      children: [
        ...
        new Table({
          ...
          rows: [
            ...
            createTableRow(["Pruebas Completadas Exitosamente", totalTests.toString()]),
            createTableRow(["🔴 Problemas Críticos Detectados", severityCounts.critical.toString()], false, 'critical'),
            createTableRow(["🟠 Problemas de Severidad Alta", severityCounts.high.toString()], false, 'high'),
            createTableRow(["🟡 Problemas de Severidad Media", severityCounts.medium.toString()], false, 'medium'),
            createTableRow(["🔵 Problemas de Severidad Baja", severityCounts.low.toString()], false, 'low'),
          ],
        }),
        ...
        new Table({
          width: { size: 100, type: WidthType.PERCENTAGE },
          rows: [
            createTableRow(["Categoría", "Severidad de Riesgo"], true),
            createTableRow(["Active Directory y Bosque", severityCounts.critical > 0 ? "Crítico" : "Sin Problemas Detectados"]),
            createTableRow(["Políticas de Cuentas de Dominio", severityCounts.high > 0 ? "Grave" : "Sin Problemas Detectados"]),
            createTableRow(["Controlador de Dominio", severityCounts.medium > 0 ? "Moderado" : "Sin Problemas Detectados"]),
            createTableRow(["Seguridad y Cumplimiento", severityCounts.low > 0 ? "Bajo" : "Sin Problemas Detectados"]),
          ],
        }),
        ...
        ...(highFindings.length > 0 ? [
          new Paragraph({
            children: [new TextRun({ 
              text: "🟠 Problemas de Severidad Alta",
              size: 36,
              bold: true,
              color: COLORS.high,
            })],
